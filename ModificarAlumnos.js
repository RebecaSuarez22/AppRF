...
function newItem (nombre,curso){

    const db = SQLite.openDatabase('db');
    db.transaction(tx => {
      tx.executeSql('INSERT INTO alumnosconimagen (nombre, clase) values (?,?)', [nombre, curso],
...
function borrarCurso(nombre){
  const db = SQLite.openDatabase('db');
  db.transaction(tx => {
    tx.executeSql('DELETE FROM alumnosconimagen WHERE clase = ? ', [nombre],
      (txObj, resultSet) => {

...
function borrarAlumno(id){
    ...
    db.transaction(tx => {
      tx.executeSql('DELETE FROM alumnosconimagen WHERE id = ? ', [id],
        (txObj, resultSet) => {
...
function editarDatos(nombre, curso,id){
    ...
    db.transaction(tx => {
      tx.executeSql('UPDATE alumnosconimagen SET nombre = ?, clase = ? WHERE id = ? ', [nombre,curso,id],
        (txObj, resultSet) => {
          if (resultSet.rowsAffected > 0) {
            let newList = this.state.data.filter(data => {
              if (data.id === id)
...
function editarAlumno ({navigation, route}) {    
    ...
    db.transaction((tx) => {
        tx.executeSql(
           'SELECT * FROM alumnosconimagen WHERE id=?',[id],
            (tx, results) => {
